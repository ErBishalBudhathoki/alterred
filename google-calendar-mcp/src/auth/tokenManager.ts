...
import { getSecureTokenPath, getAccountMode, getLegacyTokenPath, getKeysFilePath } from './utils.js';
  ...
  private async loadMultiAccountTokens(): Promise<MultiAccountTokens> {
    ...
    try {
      ...
      if (parsed.access_token || parsed.refresh_token) {
        ...
        tokens = parsed as MultiAccountTokens;
      }
    } catch (error: unknown) {
    ...
    if (saveNeeded) {
        await this.saveMultiAccountTokens(tokens);
    }

    // 2. Check Environment Credentials for injected tokens (e.g. from calendar_mcp.py)
    try {
      const envCredsPath = getKeysFilePath();
      // Check if file exists before reading
      if (await fs.access(envCredsPath).then(() => true).catch(() => false)) {
          const credsContent = await fs.readFile(envCredsPath, 'utf-8');
          const creds = JSON.parse(credsContent);
          
          // If this file contains user tokens (refresh_token), use them!
          if (creds.refresh_token) {
             ...
             const tokenCreds: Credentials = {
                 ...
                 scope: creds.scope || "https://www.googleapis.com/auth/calendar"
             };
             
             // Override/Inject tokens for the current mode
             tokens[mode] = tokenCreds;
             
             // Note: We do NOT auto-save these injected tokens to disk here, 
             // as they might be temporary or managed externally. 
             // However, if they are refreshed, setupTokenRefresh will save the new ones.
          }
      }
    } catch (e) {
        // Ignore errors reading env credentials
    }
