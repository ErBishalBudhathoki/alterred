...
async function loadCredentialsFromFile(): Promise<OAuthCredentials> {
  ...
  if (keys.installed) {
    ...
    return { client_id, client_secret, redirect_uris };
  } else if (keys.type === 'authorized_user') {
    // Service account or pre-authorized user credentials
    const { client_id, client_secret } = keys;
    return { 
        client_id, 
        client_secret, 
        redirect_uris: ['http://localhost'] // Dummy URI, not used for authorized_user flow
    };
    ...
    throw new Error('Invalid credentials file format. Expected "installed", "web", "authorized_user", or direct fields.');
  }
}
