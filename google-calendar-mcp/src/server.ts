...
export class GoogleCalendarMcpServer {
  ...
  private async handleStartupAuthentication(): Promise<void> {
    ...
    if (this.config.transport.type === 'stdio') {
      ...
      if (!hasValidTokens) {
        // Check for headless mode/disable interactive auth
        if (process.env.MCP_DISABLE_INTERACTIVE_AUTH === 'true' || process.env.CI === 'true') {
          process.stderr.write(`Authentication required but interactive mode is disabled (MCP_DISABLE_INTERACTIVE_AUTH=true). Please authenticate via the app Settings.\n`);
          process.exit(1);
        }
